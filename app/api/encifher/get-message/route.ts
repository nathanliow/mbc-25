...
export async function POST(request: NextRequest) {
  try {
    ...
    const messageToSign = await client.getMessageToSign();

    return NextResponse.json(messageToSign);
  } catch (error: any) {
    console.error("[Encifher API] Failed to get message to sign:", error);
    return NextResponse.json(
      { error: error.message || "Failed to get message to sign" },
      { status: 500 }
    );
  }
}
